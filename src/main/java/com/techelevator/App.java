...
//                        List <Integer> x = filterByTitle(filterTitle);
//                        displaySearchResults(x, TITLE_FIELD);
...
//                        displaySearchResults(filterByPriceRange(filterFromPrice, filterToPrice));
    ...
    private void displaySearchResults(List<Integer> indexes) {
        for (Integer index : indexes) {
            String info = titles.get(index) + ": " + authors.get(index) + ": " + publishedYears.get(index) + ": " + prices.get(index);
    ...
    private List<Integer> filterByPublishedYearRange(int filterFromYear, int filterToYear) {
        ...
        for (int i = 0; i < publishedYears.size(); i++) {
            if (publishedYears.get(i) >= filterFromYear && publishedYears.get(i) <= filterFromYear) {
                matches.add(i);
            }
        }
        return matches;
    ...
    private List<Integer> findMostRecentBooks() {
        int newestPublishingDate = Collections.max(publishedYears);
        return Collections.singletonList(newestPublishingDate);
    ...
    private List<Integer> findLeastExpensiveBooks() {
        List<Integer> cheapBook = new ArrayList<>();
        double cheapestBook = prices.get(0).doubleValue();
        for (BigDecimal price : prices) {
            if (price.doubleValue() < cheapestBook) {
                cheapestBook = price.doubleValue();
            }
        }
        for (int i = 0; i < prices.size(); i++) {
            if(prices.get(i).doubleValue() <= cheapestBook) {
                cheapBook.add(i);
            }
        }
        return cheapBook;
